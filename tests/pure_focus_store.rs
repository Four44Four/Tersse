...
fn btree_upsert_remove_and_rekey_are_logarithmic_map_ops() {
    ...
    btree_upsert(&mut by_key, &mut id_to_key, FocusKey::new(1.0, 1), "beta");
    btree_upsert(&mut by_key, &mut id_to_key, FocusKey::new(0.0, 0), "alpha");
    btree_upsert(&mut by_key, &mut id_to_key, FocusKey::new(2.0, 2), "gamma");

    assert_eq!(
        by_key.values().copied().collect::<Vec<_>>(),
