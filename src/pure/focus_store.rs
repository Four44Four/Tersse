...
pub fn btree_upsert<V>(
    ...
    id_to_key: &mut HashMap<String, FocusKey>,
    key: FocusKey,
    value: V,
) {
    ...
    id_to_key.insert(key.id.clone(), key.clone());
...
pub fn btree_rekey<V>(
    ...
    id_to_key.insert(id.to_string(), new_key.clone());
    by_key.insert(new_key, value);
    true
}

/// Looks up a value by id in `O(log n)` time.
pub fn btree_get<'a, V>(by_key: &'a BTreeMap<FocusKey, V>, id_to_key: &HashMap<String, FocusKey>, id: &str) -> Option<&'a V> {
